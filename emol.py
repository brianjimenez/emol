...
from gui.Uniform import Uniform
...
class EMol(DirectObject):
    ...
    def __init__(self, width, height, pdb_file1, pdb_file2, full_atom=True, debug=True):
        ...
        receptor = pdb_parser.get_structure('receptor', pdb_file1)
        ligand = pdb_parser.get_structure('ligand', pdb_file2)
        ...
        if full_atom:
            color_map = Uniform(Color.DarkOrange)
            self.load_protein_full_atom(receptor, self.receptor_node, color_map)
        else:
        ...
        if full_atom:
            color_map = Uniform(Color.Sky)
            self.load_protein_full_atom(ligand, self.ligand_node, color_map)
        else:
    ...
    def ligand_movement(self, task):
        ...
        if (self.keyMap["q"]!=0):
            self.ligand_node.setX(self.ligand_node.getX()-EMol.move_step * globalClock.getDt())
        if (self.keyMap["a"]!=0):
            self.ligand_node.setX(self.ligand_node.getX()+EMol.move_step * globalClock.getDt())
        if (self.keyMap["w"]!=0):
            self.ligand_node.setY(self.ligand_node.getY()-EMol.move_step * globalClock.getDt())
        if (self.keyMap["s"]!=0):
            self.ligand_node.setY(self.ligand_node.getY()+EMol.move_step * globalClock.getDt())
        if (self.keyMap["e"]!=0):
            self.ligand_node.setZ(self.ligand_node.getZ()-EMol.move_step * globalClock.getDt())
        if (self.keyMap["d"]!=0):
            self.ligand_node.setZ(self.ligand_node.getZ()+EMol.move_step * globalClock.getDt())
        if (self.keyMap["r"]!=0):
            self.ligand_node.setH(self.ligand_node.getH()-EMol.rotate_step * globalClock.getDt())
        if (self.keyMap["f"]!=0):
            self.ligand_node.setH(self.ligand_node.getH()+EMol.rotate_step * globalClock.getDt())
        if (self.keyMap["t"]!=0):
            self.ligand_node.setP(self.ligand_node.getP()-EMol.rotate_step * globalClock.getDt())
        if (self.keyMap["g"]!=0):
            self.ligand_node.setP(self.ligand_node.getP()+EMol.rotate_step * globalClock.getDt())
        if (self.keyMap["y"]!=0):
            self.ligand_node.setR(self.ligand_node.getR()-EMol.rotate_step * globalClock.getDt())
        if (self.keyMap["h"]!=0):
            self.ligand_node.setR(self.ligand_node.getR()+EMol.rotate_step * globalClock.getDt())
        return task.cont
    ...
    def load_protein_full_atom(self, molecule, node, color_map):
        ...
        for atom in atoms:
            ...
            a.setPos(coord[0], coord[1], coord[2])
            atom.radius = set_radii(atom) + 1.4
            a.setScale(atom.radius / EMol.radius_scale)
    ...
    def load_protein_residue(self, protein, node):
        ...
        for residue in residues:
            ...
            r.setPos(x, y, z)
            r.setScale(4.0 / EMol.radius_scale)
    ...
    def apply_color(self, a3d, atom, color_map):
        ...
        if isinstance(color_map, BFactor):
            color = color_map.get_color_by_bfactor(atom.get_bfactor())
        elif isinstance(color_map, CPK):
            color = color_map.get_color_by_element(atom.element)
        else:
            color = color_map.get_color()
    ...
    def help_on_screen(self):
        ...
        if self.help:
            i = 0
            for instruction in EMol.instructions:
